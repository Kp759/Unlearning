...
TARGET_RELATION_COMPARE_OUT_DIR="${TARGET_RELATION_COMPARE_OUT_DIR:-$PWD/results/retain_anchored_context_head/mcf/seed1_target_relation_head_compare_fix5d}"

python scripts/mcf_target_relation_head_compare_fix5d_seed1.py \
  --model-path "$MODEL_PATH" \
  --mcf-path "$MCF_PATH" \
  --view-corpus-fix5 "$RELATION_V2_CORPUS" \
  --output-dir "$TARGET_RELATION_COMPARE_OUT_DIR" \
  --dtype "${DTYPE:-bf16}" \
  --device "${DEVICE:-cuda}" \
  ...
  --mlp-hidden-dim "${RELATION_MLP_HIDDEN:-256}" \
  --mlp-dropout "${RELATION_MLP_DROPOUT:-0.1}" \
