...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/tofu_sure_author_balanced_v3_3b_test}"
...
# Same Stage1A as v1/v2.  Previous Stage1A weights may have been cleaned, so v3
# regenerates this deterministic stage from the protected Full-TOFU checkpoint.
...
PROMOTION_ROWS_PER_EXAMPLE="${PROMOTION_ROWS_PER_EXAMPLE:-1}"
MAX_PROMOTION_ROUNDS="${MAX_PROMOTION_ROUNDS:-64}"
...
# User-requested storage policy: remove model checkpoints only; preserve JSON,
# JSONL, logs, split manifests, and locked evaluations from every SURE run.
...
safe_delete_sure_checkpoint() {
  ...
  echo "DELETE SURE CHECKPOINT WEIGHTS, KEEP PARENT REPORTS: ${target_abs}"
  rm -rf -- "${target_abs}"
}

if [[ "${CLEANUP_PREVIOUS_SURE_WEIGHTS}" == "1" ]]; then
  echo "===== PRE-RUN STORAGE CLEANUP: PREVIOUS SURE WEIGHTS ONLY ====="
  ...
  echo "Previous SURE JSON/log/report files were not removed."
fi
...
for SEED in ${SEEDS}; do
  ...
  STAGE1B="${ROOT}/stage1b_progressive_v3"
  mkdir -p "${ROOT}"
  ...
  echo
  echo "===== SURE-TOFU-v3 SEED ${SEED}: STAGE1A SAME-PROMPT GA/GD ====="
  ...
  echo "===== SURE-TOFU-v3: PROGRESSIVE SPARSE SENSITIVE ROWS + RANK0 ====="
  if [[ "${SKIP_EXISTING}" != "1" || ! -f "${STAGE1B}/checkpoint/model.safetensors" ]]; then
    rm -rf "${STAGE1B}/checkpoint"
    python scripts/tofu_sure_rank0_forget_progressive_v3.py \
      ...
      --promotion-rows-per-example "${PROMOTION_ROWS_PER_EXAMPLE}" \
      --max-promotion-rounds "${MAX_PROMOTION_ROUNDS}" \
  ...
  # Freeze v3 R0 before any held-out metric enters.  We intentionally evaluate
  # R0 first to isolate whether sparse sensitivity itself restores utility.
  echo "===== V3 CHECKPOINT FROZEN; LOCKED HELD-OUT EVALUATION STARTS ====="
  EVAL_ARGS=(
    --model-dir "${STAGE1B}/checkpoint"
    --eval-dir "${EVAL_DIR}"
    --reference-model-dir "${FULL_TOFU_MODEL}"
    --output "${ROOT}/locked_eval_v3_r0.json"
    --seed "${SEED}"
    --dtype "${DTYPE}"
...
if [[ ! -f "${FULL_TOFU_MODEL}/model.safetensors" || ! -f "${FULL_TOFU_MODEL}/config.json" ]]; then
  echo "FATAL: protected Full-TOFU checkpoint missing after v3 cleanup" >&2
  exit 93
fi

echo
echo "SURE-TOFU-v3 complete. Sparse row selection used only training-visible direct forget QAs."
echo "Previous/current SURE checkpoint weights were removed; JSON/report files were kept."
