...
if [[ "$#" -ne 4 ]]; then
  echo "Usage: bash scripts/run_rwku_sure_hidden_direction_w1k.sh MODEL STEPHEN_KING_ATOMIC_CORPUS_DIR REAL_WIKIPEDIA_DIR SOURCE_HEAD_ONLY_RUN_DIR" >&2
  exit 2
fi
...
configuration="config/rwku/sure_head_hidden_direction_w1k_seed0.json"
training_bundle="${corpus_dir}/generated_training_bundle.json"
generator_receipt="${corpus_dir}/generator_receipt.json"
output_root="${RWKU_HD_W1K_OUTPUT_ROOT:-outputs/rwku_hd_w1k}"
...
if [[ -e "${run_dir}" ]]; then
  echo "Refusing to overwrite hidden-direction run: ${run_dir}" >&2
  echo "Choose a new RWKU_HD_W1K_OUTPUT_ROOT; never reuse a checkpoint receipt." >&2
  exit 2
fi
...
# Fresh v3 experiment identity. This is explicitly post-hoc Stephen King
# development because official Stephen King RWKU results were observed in v2.
# The learner itself receives no official RWKU records.
...
"${python_bin}" scripts/rwku_sure_hidden_direction_w1k.py \
  --model-path "${model}" \
...
echo "RWKU-H+HD-W1K checkpoint frozen: ${run_dir}"
echo "Development-only Stephen King v3. Inspect hidden-direction, frozen-head proxy, and W1K reports before any evaluation."
