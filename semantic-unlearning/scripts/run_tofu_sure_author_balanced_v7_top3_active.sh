...
TMP="${SCRIPT_DIR}/.run_v7_top3_active_${$}.sh"
trap 'rm -f -- "${TMP}"' EXIT
...
new_script = "python scripts/tofu_sure_sparse_lm_gagd_top3_v7.py"
old_root = 'OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/tofu_sure_author_balanced_v7_sparsegagd_active_3b_test}"'
new_root = 'OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/tofu_sure_author_balanced_v7_top3_active_3b_test}"'
...
text = text.replace(
    ...
    "# Stage 1: GA only on top-3 rare/content sensitive answer rows per QA; same-prompt GD/KL protection.",
    1,
)
text = text.replace(
    'echo "===== SURE-TOFU V7 SEED ${SEED}: STAGE1 SPARSE LM-HEAD GA/GD ====="',
    'echo "===== SURE-TOFU V7-TOP3 SEED ${SEED}: STAGE1 TOP3 SENSITIVE LM-HEAD GA/GD ====="',
    1,
)
