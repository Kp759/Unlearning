...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/aws_mquake_sure_no_neutral_gagd_aggressive_3b}"
...
python scripts/mquake_no_neutral_stage1_gagd.py \
  ...
  --ga-weight 2.0 --gd-weight 1.0 --grad-clip 1.0 --dtype "${DTYPE}" --device-map "${DEVICE_MAP}"
...
python scripts/mquake_zero_unlearn_official_eval.py \
  ...
  --method "SURE MQuAKE no-neutral Emb+LM GA/GD plus sensitive-row repair" \
...
python scripts/mquake_zero_unlearn_official_eval.py \
  ...
  --method "SURE MQuAKE no-neutral Emb+LM GA/GD Stage1 posthoc" \
  --unlearn-num "${FORGET_NUM}" --retain-num "${RETAIN_NUM}" --seed "${SEED}" \
  --batch-size "${EVAL_BATCH_SIZE}" --dtype "${DTYPE}" --device-map "${DEVICE_MAP}"

# MQuAKE-specific multi-hop leakage is strictly post-selection.
