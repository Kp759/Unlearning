...
MODEL="${1:?Usage: bash scripts/run_mquake_sure_contextual_sensitive_suppression_rank4.sh MODEL [MQUAKE_JSON]}"
MQUAKE="${2:-data/MQuAKE-CF-3k-v2.json}"
SEED="${MQUAKE_SEED:-1}"
FORGET_NUM="${MQUAKE_FORGET_NUM:-50}"
RETAIN_NUM="${MQUAKE_RETAIN_EVAL_NUM:-1000}"
ROOT="${OUTPUT_ROOT:-outputs/aws_mquake_sure_sensitive_suppression_rank4/seed${SEED}}"
...
OUT="${ROOT}/contextual_sensitive_suppression"
...
rm -rf "${OUT}"
python scripts/mquake_sure_contextual_sensitive_suppression.py \
  ...
  --active-lr "${LORA_ACTIVE_LR:-0.00002}" \
  --l2 "${LORA_L2:-0.000001}" \
  --target-eff-max 0 \
  ...
  2>&1 | tee "${ROOT}/stage2_sensitive_suppression_train.log"
...
python scripts/mquake_zero_unlearn_official_eval.py \
  ...
  --method "SURE Emb+LM Stage1 + rank4 contextual sensitive suppression" \
