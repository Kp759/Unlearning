...
from static_overlap_orthogonal_protocol_v5 import METHOD, locality_prompt_specs
...
def choose_basis_examples(examples, synthetic_ids, hard_ids, limit):
    ...
    chosen = []
    # Reserve one locality direction for every forgotten subject, then devote
    # the rest of the expanded basis to the accumulated worst-case anchors.
    # v4 capped hard anchors at half the basis (64 of 230 by its last gate), so
    # most constraints that triggered rollback were absent from the projector.
    subject_count = len({eid.rsplit("_", 1)[0] for eid in synthetic_ids})
    hard_limit = min(len(hard_ids), max(0, limit - subject_count))
...
def build_protected_basis(editor, examples, synthetic_ids, hard_ids, plan):
    ...
    return basis, {"requested_examples": len(selected), "numerical_rank": len(basis),
                   "selected_hard_anchors": sum(eid in hard for eid in selected_ids),
                   "selected_subject_locality_queries": sum(eid in synthetic for eid in selected_ids),
...
def orthogonal_step(editor, pairs, background, references, config, plan, basis):
    ...
    projected_forget = project_away(forget_gradient, basis)
    projected_retain = project_away(retain_gradient, basis)
    ratio = float(projected_forget.norm() / forget_gradient.norm().clamp_min(1e-30))
    # Project the complete update.  v4 projected GA and then added raw GD,
    # which could reintroduce a component along a protected direction.
    combined = projected_forget + projected_retain
    max_forget_overlap = (float((basis @ projected_forget.float()).abs().max())
                          if basis.numel() else 0.)
    max_total_overlap = (float((basis @ combined.float()).abs().max())
                         if basis.numel() else 0.)
    ...
    if (not torch.isfinite(norm) or float(norm) == 0.
        ...
        return {"mode": "orthogonal_paired_ga_gd", "accepted": False,
                ...
                "projected_forget_gradient_norm": float(projected_forget.norm()),
                "projected_retain_gradient_norm": float(projected_retain.norm()),
                "forget_residual_ratio": ratio,
                "projected_forget_max_abs_overlap": max_forget_overlap,
                "complete_update_max_abs_overlap": max_total_overlap,
...
def fit(editor, examples, references, config, plan, output, *, source, data, tokenizer):
    ...
    def report():
        return {"method": METHOD, "exploratory": True, "stop_reason": stop,
                ...
                "complete_ga_gd_update_is_projected": True,
                "exact_declared_preservation_limits_used_for_fitting": True,
