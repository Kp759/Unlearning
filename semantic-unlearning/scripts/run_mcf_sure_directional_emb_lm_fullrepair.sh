...
# Rank 0 in sure_stage2_sparse_repair.py means unrestricted selected-row delta.
# Supplying ONLY rank 0 disables the low-rank/LoRA-style candidate path.
python -u scripts/sure_stage2_sparse_repair.py \
  --dataset mcf \
  ...
  --seed "$SEED" \
  --forget-num "$FORGET_NUM" \
  --mcf-sensitive-field target_true \
  --mcf-reference-field target_new \
  --candidate-ranks 0 \
...
printf '\nFinished directional Emb+LM -> full-row LM-head repair.\n'
