...
if [[ $# -ne 10 ]]; then
  echo "usage: $0 CLEAN_WRITER_OUTPUT_DIR V3_2_OUTPUT_DIR V3_4_OUTPUT_DIR REJECTED_V3_5_OUTPUT_DIR V3_5_1_FORENSICS_OUTPUT_DIR REJECTED_V3_5_2_OUTPUT_DIR REJECTED_V3_5_3_OUTPUT_DIR REJECTED_V3_5_4_OUTPUT_DIR PASSED_V3_5_5_OUTPUT_DIR OUTPUT_DIR" >&2
  exit 2
fi
...
if [[ -e "${RUN_OUTPUT_DIR}" ]]; then
  echo "output path already exists; choose a fresh V3.6 directory: ${RUN_OUTPUT_DIR}" >&2
  exit 2
fi
...
exec bash "${PROJECT_DIR}/slurm/run_mcf_embedding_keyed_neuron_v3_6_full_preservation_seed1_3b.slurm"
