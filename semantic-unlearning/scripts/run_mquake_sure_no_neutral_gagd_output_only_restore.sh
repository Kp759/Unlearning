...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/aws_mquake_sure_no_neutral_gagd_output_only_restore_3b}"
...
STAGE1="${ROOT}/stage1_emb_lm_gagd_output_only_restore"
...
rm -rf "${STAGE1}"
python scripts/mquake_no_neutral_stage1_gagd.py \
  --model-path "${MODEL}" --training-visible-path "${VISIBLE}" --split-manifest "${MANIFEST}" \
  --output-dir "${STAGE1}" --seed "${SEED}" --forget-num "${FORGET_NUM}" \
  --steps 600 --batch-size 1 --cache-batch-size 8 --emb-lm-lr 0.0001 \
  --ga-weight 2.0 --gd-weight 1.0 --grad-clip 1.0 \
  --restoration-mode output_only --dtype "${DTYPE}" --device-map "${DEVICE_MAP}"
...
python scripts/mquake_zero_unlearn_official_eval.py \
  ...
  --method "SURE MQuAKE no-neutral Emb+LM GA/GD output-only restoration plus sensitive-row repair" \
...
python scripts/mquake_zero_unlearn_official_eval.py \
  ...
  --method "SURE MQuAKE no-neutral Emb+LM GA/GD output-only restoration Stage1 posthoc" \
  --unlearn-num "${FORGET_NUM}" --retain-num "${RETAIN_NUM}" --seed "${SEED}" \
  --batch-size "${EVAL_BATCH_SIZE}" --dtype "${DTYPE}" --device-map "${DEVICE_MAP}"
...
s=json.load(open(f"{r}/stage1_emb_lm_gagd_output_only_restore/vocabulary_restoration.json"))
print("RESTORATION",json.dumps(s,sort_keys=True))
PY
