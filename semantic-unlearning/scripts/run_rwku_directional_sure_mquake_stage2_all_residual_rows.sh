...
if [[ "$#" -ne 3 ]]; then
  echo "Usage: bash scripts/run_rwku_directional_sure_mquake_stage2_all_residual_rows.sh MODEL CORPUS_DIR WIKIPEDIA_DIR" >&2
  exit 2
fi
...
python scripts/rwku_directional_sure_mquake_stage2_all_residual_rows.py \
  --model-path "$MODEL" \
