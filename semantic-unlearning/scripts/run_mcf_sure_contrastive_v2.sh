...
MODEL="${1:?Usage: bash scripts/run_mcf_sure_contrastive_v2.sh MODEL [MCF_JSON]}"
MCF="${2:-data/multi_counterfact.json}"
WIKIDATA_DIR="${WIKIDATA_DIR:-data/wikidata}"
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mcf_sure_contrastive_v2}"
...
for SEED in "${SEEDS[@]}"; do
  ...
  python scripts/mcf_zero_unlearn_official_eval.py \
    --model-dir "${LEARNER}/checkpoint" --mcf-path "${MCF}" --wikidata-dir "${WIKIDATA_DIR}" \
    --out "${FINAL_EVAL}" --unlearn-num "${FORGET_NUM}" --retain-num "${RETAIN_EVAL_NUM}" \
    --seed "${SEED}" --sample-mode official --dtype "${DTYPE}" --device-map "${DEVICE_MAP}"

  python scripts/annotate_ppl_provenance.py --eval-json "${BASE_EVAL}" --model-dir "${MODEL}" --wikidata-dir "${WIKIDATA_DIR}"
  python scripts/annotate_ppl_provenance.py --eval-json "${FINAL_EVAL}" --model-dir "${LEARNER}/checkpoint" --wikidata-dir "${WIKIDATA_DIR}"

  python scripts/evaluate_mcf_target_true_sensitive.py \
    --base-eval-json "${BASE_EVAL}" --post-eval-json "${FINAL_EVAL}" \
...
for k in [
    "stage1_selected_scale","stage1_selection_mode",
    "stage1_direct_failures_after_materialization","stage2_mode",
    "final_direct_failures","final_minimum_logit_margin",
    "final_minimum_sensitive_nll_increase","final_retain_action",
    "retain_action_budget","final_total_delta_norm","max_total_delta_norm",
    "final_forget_nonsensitive_kl_proxy","max_forget_nonsensitive_kl",
    "final_guard_pass",
]: print(f"{k}: {d.get(k)}")
PY
...
echo "MCF contrastive SURE-LM v2 complete: ${OUTPUT_ROOT}"
