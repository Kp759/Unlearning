...
# Stage 2: protected subspace LM-head repair using the official-direct
#          sequence margin NLL(target_true)-NLL(target_new).
#
# Usage:
#   OUT_ROOT=outputs/mcf_protected_mcf_margin_seed1 \
#   bash scripts/run_mcf_sure_protected_subspace_mcf_margin.sh \
#     /path/to/base-model \
...
#   MCF_MARGIN=0.05 PROTECTED_MCF_MARGIN_FLOOR=0.0
...
OUT_ROOT="${OUT_ROOT:-outputs/mcf_protected_mcf_margin_seed${SEED}}"
...
STAGE2_DIR="$OUT_ROOT/stage2_protected_mcf_margin"
...
printf '\n=== STAGE 2: protected MCF sequence-margin repair ===\n'
python -u scripts/mcf_sure_protected_subspace_stage2_mcf_margin.py \
...
printf '\nProtected MCF-margin SURE finished.\n'
printf 'Stage 1: %s\n' "$STAGE1_CKPT"
printf 'Final  : %s\n' "$FINAL_CKPT"
printf 'Root   : %s\n' "$OUT_ROOT"
