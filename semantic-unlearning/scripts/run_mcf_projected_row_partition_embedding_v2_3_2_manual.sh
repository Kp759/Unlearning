...
# Training-only MCF V2.3.2. The split builder alone sees MCF_PATH.
# Required environment: MODEL_PATH, WIKIDATA_DIR, MCF_PATH
# Usage: bash scripts/run_mcf_projected_row_partition_embedding_v2_3_2_manual.sh OUTPUT_DIR

OUTPUT_DIR=${1:?fresh V2.3.2 output directory required}
...
if [ -e "$OUTPUT_DIR" ]; then
  echo "ERROR: V2.3.2 output already exists: $OUTPUT_DIR" >&2
  exit 1
fi

REGISTRY=protocols/mcf_projected_row_partition_embedding_v2_3_2_registry.json
...
printf '\nMCF V2.3.2 training-only run finished.\n'
