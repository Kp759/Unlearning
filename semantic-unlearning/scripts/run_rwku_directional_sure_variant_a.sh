...
if [[ "$#" -ne 3 ]]; then
  echo "Usage: bash scripts/run_rwku_directional_sure_variant_a.sh MODEL CORPUS_DIR WIKIPEDIA_DIR" >&2
  exit 2
fi
...
CONFIG="config/rwku/directional_sure_variant_a_seed0.json"
OUTPUT_ROOT="${RWKU_DIRECTIONAL_SURE_VARIANT_A_OUTPUT_ROOT:-outputs/rwku_directional_sure_variant_a}"
...
if [[ -e "$RUN" ]]; then
  echo "Refusing to overwrite Directional SURE Variant A run: $RUN" >&2
  exit 2
fi
...
python scripts/rwku_directional_sure_variant_a.py \
  --model-path "$MODEL" \
...
echo "RWKU Directional SURE Variant A development run complete: $RUN"
