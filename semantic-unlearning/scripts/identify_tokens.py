...
This version uses only frequency analysis:

  T_f = frequency_tokens

Branch 1 — FREQUENCY ANALYSIS:
  Tokens that appear frequently in D_f but rarely in D_r

The original probe / semantic code is kept commented out below.
Saved output:
...
def find_frequency_tokens(
    ...
    for tid, f_count in freq_forget.items():
        ...
        retain_ratio = r_count / n_retain if n_retain > 0 else 0.0
...
# ── Branch 2: Probe Differential Scoring (COMMENTED OUT FOR FREQ-ONLY) ──────
...
# ── Union (COMMENTED OUT FOR FREQ-ONLY) ──────────────────────────────────────
# def union_tokens(
...
def main():
    ...
    # ORIGINAL SEMANTIC ARGUMENTS COMMENTED OUT
    # parser.add_argument(
    #     "--best-layer",
    #     type=int,
    #     default=None,
    ...
    # Keep these only for CLI compatibility in freq-only mode
    parser.add_argument(
        "--best-layer",
        type=int,
        default=None,
    ...
    # ── Restore probe setup (COMMENTED OUT FOR FREQ-ONLY) ─────────────────
    ...
    # )
    # print(
    #     f"[Layer] Using layer {best_layer} | AUC={layer_accuracies[best_layer]:.4f}"
    #     + (" (manual override)" if args.best_layer is not None else " (auto-selected)")
    # )
    #
    ...
    # ── Probe threshold (COMMENTED OUT FOR FREQ-ONLY) ─────────────────────
    # threshold = args.threshold or cfg.get("probe_analysis", {}).get("threshold", 0.10)
    threshold = None
    ...
    # ── Branch 2 + Union (COMMENTED OUT FOR FREQ-ONLY) ────────────────────
    ...
    n_probe = 0
    print(f"  Both:      {n_both} tokens")
    print(f"  Frequency: {n_freq} tokens")
    print(f"  Probe:     {n_probe} tokens")
    print(f"  Total T_f: {len(semantic_tokens)} tokens")

    # ── Save semantic+freq output (COMMENTED OUT FOR FREQ-ONLY) ───────────
    # output = {
    ...
    # ── Semantic+freq plot (COMMENTED OUT FOR FREQ-ONLY) ──────────────────
