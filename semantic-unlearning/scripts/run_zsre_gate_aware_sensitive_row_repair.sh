...
FORGET_REGRESSION_MARGIN="${FORGET_REGRESSION_MARGIN:-0.02}"
PROTECTED_MARGIN_CAP="${PROTECTED_MARGIN_CAP:-0.05}"
ACTIVE_HINGE_WEIGHT="${ACTIVE_HINGE_WEIGHT:-2.0}"
FORGET_REGRESSION_HINGE_WEIGHT="${FORGET_REGRESSION_HINGE_WEIGHT:-2.0}"
...
for seed in "${SEED_ARRAY[@]}"; do
  ...
  RUN_ARGS=(
    ...
    --forget-regression-margin "${FORGET_REGRESSION_MARGIN}"
    --protected-margin-cap "${PROTECTED_MARGIN_CAP}"
    --active-hinge-weight "${ACTIVE_HINGE_WEIGHT}"
    --forget-regression-hinge-weight "${FORGET_REGRESSION_HINGE_WEIGHT}"
