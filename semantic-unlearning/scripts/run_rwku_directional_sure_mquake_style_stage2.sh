...
if [[ "$#" -ne 3 ]]; then
  echo "Usage: bash scripts/run_rwku_directional_sure_mquake_style_stage2.sh MODEL CORPUS_DIR WIKIPEDIA_DIR" >&2
  exit 2
fi
...
CONFIG="config/rwku/directional_sure_mquake_style_stage2_seed0.json"
OUTPUT_ROOT="${RWKU_DIRECTIONAL_SURE_MQUAKE_STYLE_STAGE2_OUTPUT_ROOT:-outputs/rwku_directional_sure_mquake_style_stage2}"
...
if [[ -e "$RUN" ]]; then
  echo "Refusing to overwrite RWKU MQuAKE-style Stage2 run: $RUN" >&2
  exit 2
fi
...
python scripts/rwku_directional_sure_mquake_style_stage2.py \
  --model-path "$MODEL" \
...
echo "RWKU Directional SURE + MQuAKE-style Stage2 development run complete: $RUN"
