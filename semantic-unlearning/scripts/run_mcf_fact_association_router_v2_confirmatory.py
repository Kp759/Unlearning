#!/usr/bin/env python3
"""Train frozen Router V2 on confirmatory MCF seeds 2--10.\n\nSeed 1 is development-only. Architecture and hyperparameters are frozen from\nthe seed-1 development snapshot; only the official split/RNG seed changes.\n"""
...
def main(argv=None):
    ...
    if args.forget_num != 50 or not (2 <= int(args.seed) <= 10):
        raise ValueError(
            "Confirmatory MCF experiment requires forget_num=50 and seed in [2, 10]."
        )
    if int(args.layer) != int(PLAN["layer"]):
        raise ValueError("Confirmatory MCF freezes intervention layer to seed-1 value")
    if float(args.gate_slack) != float(PLAN["gate_slack"]):
        raise ValueError("Confirmatory MCF freezes legacy gate_slack to seed-1 value")
    if float(args.min_dev_route_recall) != float(PLAN["min_development_route_recall"]):
        raise ValueError(
            "Confirmatory MCF freezes development route-recall floor to seed-1 value"
        )
    ...
    manifest = {
        ...
        "development_seed": 1,
        "confirmatory_seed": int(args.seed),
        "hyperparameters_frozen_from_seed1": True,
