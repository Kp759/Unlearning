...
cd "${project_dir}"
if [[ "$#" -ne 4 ]]; then
  echo "Usage: bash scripts/run_rwku_sure_hidden_direction_v31_w1k.sh MODEL STEPHEN_KING_ATOMIC_CORPUS_DIR REAL_WIKIPEDIA_DIR SOURCE_HEAD_ONLY_RUN_DIR" >&2
  exit 2
fi
model="$1"
corpus_dir="$2"
...
output_root="${RWKU_HD_V31_W1K_OUTPUT_ROOT:-outputs/rwku_hd_v31_w1k}"
...
if [[ -e "${run_dir}" ]]; then
  echo "Refusing to overwrite v3.1 hidden-direction run: ${run_dir}" >&2
  echo "Choose a new RWKU_HD_V31_W1K_OUTPUT_ROOT; never reuse a checkpoint receipt." >&2
  exit 2
fi
download_args=()
...
"${python_bin}" scripts/rwku_experiment.py \
  ...
  "${download_args[@]}"
"${python_bin}" scripts/rwku_sure_hidden_direction_v31_w1k.py \
  --model-path "${model}" \
...
echo "RWKU-H+HD-v3.1-W1K checkpoint frozen: ${run_dir}"
echo "Development-only Stephen King v3.1. Inspect answer-level frozen-base-head, atomic, norm, and W1K reports before any evaluation."
