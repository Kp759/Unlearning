...
AUTO_RECOVER="${SURE_CONTEXT_V3_AUTO_RECOVER:-1}"
...
for SEED in "${SEEDS[@]}"; do
  ...
  python scripts/mquake_sure_stage2_context_protected_v3.py \
    --model-path "${STAGE1}" \
  ...
  if [[ "${FINAL_PASS}" != "1" && "${AUTO_RECOVER}" == "1" ]]; then
    echo "===== TRAINING GATE FAILED AFTER MATERIALIZATION: RUN DETERMINISTIC LOW-PRECISION RECOVERY ====="
