#!/usr/bin/env python3
"""Official ZeroUnlearn-parity evaluation for RSNR-V1B PreHead StandardUnlearn.

Reports exact ZeroUnlearn Eff/Gen labels from post_rewrite_success and
post_paraphrase_success, while retaining SensitivePref, IDK, true-drop,
generation leakage/abstention, routing, specificity, and PPL diagnostics.
"""
...
def load_and_validate_artifacts(run_dir: Path, locked_forget, manifest):
    ...
    adapter_path = method_dir / "rsnr_prehead_standard_unlearn_adapter.pt"
    adapter = torch.load(adapter_path, map_location="cpu", weights_only=False)
    sidecar = _load_json(method_dir / "relation_scoped_prehead_standard_unlearn.json")
    completion = _load_json(method_dir / "completion.json")
    report = _load_json(method_dir / "rsnr_v1b_prehead_standard_unlearn.json")
    ...
    if adapter.get("target_new_gradient") is not False:
        raise RuntimeError("target_new must have been detached during training")
    if completion.get("training_gate_passed") is not True:
        raise RuntimeError("training artifact did not pass all four training-safe conditions")
...
def main() -> None:
    ...
    try:
        ...
        # Critical: paper-facing Eff/Gen use EXACT ZeroUnlearn raw success labels.
        ...
        result = {
            "method": "rsnr_v1b_prehead_standard_unlearn",
            "variant": v1b.VARIANT,
            "protocol": PROTOCOL,
        ...
        print(f"RSNR-V1B PreHead official result: {out}")
    finally:
        hook.remove()
