...
def main():
    ...
    Bs=lora_B_params(model); originals={n:p.detach().clone() for n,p in Bs.items()}; scales=sorted(set([float(x) for x in a.candidate_scales.split(',')]+[0.,1.])); best=None; selected=0.; reports=[]
    for sc in scales:
        with torch.no_grad():
        ...
        reports.append({'scale':sc,'Eff':sm['Eff'],'correct':cor}); key=(float(sm['Eff']),cor,sc)
        if best is None or key<best: best=key; selected=sc
    with torch.no_grad():
    ...
    nnr.write(out/'summary.json',{'method':'SURE-contextual-LoRA-no-neutral','seed':a.seed,'target_new_seen':False,'Unknown_used':False,'retain_seen':0,'PPL_seen_during_selection':False,'layers':layers,'rank':a.rank,'alpha':a.alpha,'target_modules':['o_proj','down_proj'],'base':base_sum,'stage1':sum1,'phase1':phase1,'active_before_phase2':len(active_cases),'phase2':phase2,'selected_scale':selected,'selected':final_sum,'scale_reports':reports,'checkpoint':str(ckpt.resolve()),'sampling':manifest.get('sampling')})
    print('\n===== CONTEXTUAL LoRA NO-NEUTRAL ====='); print('Base Eff:',base_sum['Eff']); print('Stage1 Eff:',sum1['Eff']); print('Active before phase2:',len(active_cases)); print('Selected Eff:',final_sum['Eff']); print('Selected scale:',selected); print('Unknown/target_new: NO'); print('checkpoint:',ckpt)
