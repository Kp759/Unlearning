...
# FIXED SHARED architecture defaults. Keep equal to MCF runner.
...
for SEED in "${SEEDS[@]}"; do
  ...
  echo "===== ZSRE SEED ${SEED}: LOCKED NO-NEUTRAL SPLIT ====="
  ...
  echo "===== ZSRE SEED ${SEED}: FIXED SHARED STAGE 1 ====="
  ...
  python scripts/annotate_ppl_provenance.py \
    --eval-json "${FINAL}" --model-dir "${STAGE2}/checkpoint" --wikidata-dir "${WIKIDATA_DIR}"
done
