...
def strip_hidden(rows: Sequence[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    return [{k: v for k, v in row.items() if k != "hidden"} for row in rows]
...
def pair_key(row: Mapping[str, Any]) -> Tuple[int, int, int]:
    return (int(row["case_id"]), int(row.get("prompt_index", 0)), int(row["token_index"]))


def macro_metric(records: Sequence[Mapping[str, Any]], rows: Sequence[Mapping[str, Any]], prompt_type: str) -> Optional[float]:
...
def token_position_summary(rows: Sequence[Mapping[str, Any]], prompt_type: str) -> Dict[str, Any]:
    ...
    for key in ("0", "1", "2+"):
        ...
        out[key] = {
            ...
            "forget_margin": percentile_summary([float(x["forget_margin"]) for x in vals]),
        }
    return out
...
def exact_changed_rows(current: torch.Tensor, base: torch.Tensor) -> torch.Tensor:
    ...
    # Both tensors are CPU snapshots in checkpoint dtype.  Untouched rows should
    # be bit-exact because v3 materializes only selected vocabulary rows.
    return (current != base).any(dim=1)


def delta_row_norms(current: torch.Tensor, base: torch.Tensor, block: int = 4096) -> torch.Tensor:
...
def evaluate_ppl(model, tok, wikidata_dir: Path) -> Optional[float]:
    ...
    if text is None:
        return None
    return float(official_perplexity(model, tok, text, first_device(model), max_input_length=100))


def matched(rows: Sequence[Mapping[str, Any]]) -> Dict[Tuple[int, str, int, int], Mapping[str, Any]]:
...
def paired_gen_analysis(
    ...
    final_rewrite = {pair_key(r): r for r in final_rows if r["prompt_type"] == "rewrite"}
    final_gen = {pair_key(r): r for r in final_rows if r["prompt_type"] == "atomic_gen"}
    base_rewrite = {pair_key(r): r for r in base_rows if r["prompt_type"] == "rewrite"}
    base_gen = {pair_key(r): r for r in base_rows if r["prompt_type"] == "atomic_gen"}
    ...
    for pk in sorted(set(final_rewrite) & set(final_gen)):
        ...
        if isinstance(h_d, torch.Tensor) and isinstance(h_g, torch.Tensor):
            cos_final = float(torch.nn.functional.cosine_similarity(h_d[None], h_g[None]).item())
        if isinstance(hb_d, torch.Tensor) and isinstance(hb_g, torch.Tensor):
            cos_base = float(torch.nn.functional.cosine_similarity(hb_d[None], hb_g[None]).item())
        ...
        paired_rows.append({
            ...
            "margin_gap_atomicgen_minus_direct": float(g["forget_margin"]) - float(d["forget_margin"]),
            "base_hidden_cosine_direct_atomicgen": cos_base,
            "final_hidden_cosine_direct_atomicgen": cos_final,
            "direct_target_logit_shift_base_to_stage1": float(sd["target_logit"]) - float(bd["target_logit"]),
            "direct_target_logit_shift_stage1_to_final": float(d["target_logit"]) - float(sd["target_logit"]),
            "direct_target_logit_shift_base_to_final": float(d["target_logit"]) - float(bd["target_logit"]),
            "gen_target_logit_shift_base_to_stage1": float(sg["target_logit"]) - float(bg["target_logit"]),
            "gen_target_logit_shift_stage1_to_final": float(g["target_logit"]) - float(sg["target_logit"]),
            "gen_target_logit_shift_base_to_final": float(g["target_logit"]) - float(bg["target_logit"]),
        })
    ...
    def group_summary(rows):
        return {
            "n": len(rows),
            "final_atomicgen_margin": percentile_summary([r["final_atomicgen_margin"] for r in rows]),
            "final_direct_margin": percentile_summary([r["final_direct_margin"] for r in rows]),
            "margin_gap_atomicgen_minus_direct": percentile_summary([r["margin_gap_atomicgen_minus_direct"] for r in rows]),
            "base_hidden_cosine": percentile_summary([r["base_hidden_cosine_direct_atomicgen"] for r in rows if r["base_hidden_cosine_direct_atomicgen"] is not None]),
            "final_hidden_cosine": percentile_summary([r["final_hidden_cosine_direct_atomicgen"] for r in rows if r["final_hidden_cosine_direct_atomicgen"] is not None]),
            "direct_target_logit_shift_base_to_final": percentile_summary([r["direct_target_logit_shift_base_to_final"] for r in rows]),
            "gen_target_logit_shift_base_to_final": percentile_summary([r["gen_target_logit_shift_base_to_final"] for r in rows]),
            "gen_target_logit_shift_stage1_to_final": percentile_summary([r["gen_target_logit_shift_stage1_to_final"] for r in rows]),
        }
    ...
    *,
    ...
    for key in common:
        ...
        details.append({
            ...
            "target_stage2_incremental_norm": float(stage2_incremental_norms.get(tid, 0.0)),
            "winner_stage2_incremental_norm": float(stage2_incremental_norms.get(win, 0.0)),
    ...
    def status_counts(rows):
        return {
            "n": len(rows),
            "base_accuracy_percent": None if not rows else 100.0 * sum(r["base_correct"] for r in rows) / len(rows),
            "stage1_accuracy_percent": None if not rows else 100.0 * sum(r["stage1_correct"] for r in rows) / len(rows),
            "final_accuracy_percent": None if not rows else 100.0 * sum(r["final_correct"] for r in rows) / len(rows),
        }
    ...
    x = np.asarray([r["target_final_delta_norm"] for r in base_correct], dtype=np.float64)
    y = np.asarray([1.0 if r["base_to_final_lost"] else 0.0 for r in base_correct], dtype=np.float64)
    corr = None
    ...
    return {
        "token_transition_counts": {
            "base_correct_final_wrong": sum(r["base_to_final_lost"] for r in details),
            "base_wrong_final_correct": sum(r["base_to_final_gained"] for r in details),
            "base_correct_stage1_wrong": sum(r["base_to_stage1_lost"] for r in details),
            "stage1_correct_final_wrong": sum(r["stage1_to_final_lost"] for r in details),
        },
        "target_row_overlap": {
        ...
        "new_failure_target_delta_norm": percentile_summary([r["target_final_delta_norm"] for r in lost]),
        "new_failure_winner_delta_norm": percentile_summary([r["winner_final_delta_norm"] for r in lost]),
        "base_correct_preserved_target_delta_norm": percentile_summary([
            r["target_final_delta_norm"] for r in details if r["base_correct"] and r["final_correct"]
        ]),
...
def main() -> None:
    ...
    llama_like = is_llama_like(None, tok)
    ...
    forget_cases = [
        ...
        for c in official.expand_prediction_cases(
            r, tok, llama_like=llama_like, prompt_types=("rewrite", "atomic_gen")
        )
    ]
    retain_cases = [
        c
        ...
        for c in official.expand_prediction_cases(
            r, tok, llama_like=llama_like, prompt_types=("rewrite",)
        )
    ]

    print(f"Forensics split: forget atomic facts={len(forget_records)} forget token cases={len(forget_cases)} retain atomic facts={len(retain_records)} retain token cases={len(retain_cases)}")

    # BASE -----------------------------------------------------------------
    print("===== FORENSICS: BASE =====")
    base_model = load_model(a.base_model_path, a.dtype, a.device_map)
    # Re-evaluate llama_like using the actual model to exactly mirror official code.
    llama_like = is_llama_like(base_model, tok)
    base_forget = score_cases(base_model, tok, forget_cases, llama_like=llama_like, batch_size=a.batch_size, include_hidden=True)
    base_retain = score_cases(base_model, tok, retain_cases, llama_like=llama_like, batch_size=a.batch_size, include_hidden=False)
    base_ppl = evaluate_ppl(base_model, tok, Path(a.wikidata_dir)) if a.with_ppl else None
    base_w = base_model.get_output_embeddings().weight.detach().cpu().clone()
    base_stage = summarize_stage("base", forget_records, retain_records, base_forget, base_retain, base_ppl)
    print(base_stage)
    unload(base_model)
    ...
    stage1_forget = score_cases(stage1_model, tok, forget_cases, llama_like=llama_like, batch_size=a.batch_size, include_hidden=False)
    stage1_retain = score_cases(stage1_model, tok, retain_cases, llama_like=llama_like, batch_size=a.batch_size, include_hidden=False)
    stage1_ppl = evaluate_ppl(stage1_model, tok, Path(a.wikidata_dir)) if a.with_ppl else None
    ...
    stage1_changed_ids = torch.nonzero(stage1_changed, as_tuple=False).flatten().tolist()
    stage1_selected_snapshot = stage1_w.index_select(0, torch.tensor(stage1_changed_ids, dtype=torch.long)).clone() if stage1_changed_ids else torch.empty((0, base_w.shape[1]), dtype=base_w.dtype)
    stage1_stage = summarize_stage("stage1_v3", forget_records, retain_records, stage1_forget, stage1_retain, stage1_ppl)
    print(stage1_stage)
    unload(stage1_model)
    ...
    final_forget = score_cases(final_model, tok, forget_cases, llama_like=llama_like, batch_size=a.batch_size, include_hidden=True)
    final_retain = score_cases(final_model, tok, retain_cases, llama_like=llama_like, batch_size=a.batch_size, include_hidden=False)
    final_ppl = evaluate_ppl(final_model, tok, Path(a.wikidata_dir)) if a.with_ppl else None
    ...
    stage2_incremental_norms: Dict[int, float] = {}
    if stage1_changed_ids:
        ids = torch.tensor(stage1_changed_ids, dtype=torch.long)
        final_selected = final_w.index_select(0, ids)
        inc = (final_selected.float() - stage1_selected_snapshot.float()).norm(dim=1)
        stage2_incremental_norms = {int(tid): float(v) for tid, v in zip(stage1_changed_ids, inc.tolist()) if float(v) > 0.0}
        stage2_frob = float(inc.double().square().sum().sqrt().item())
        stage2_changed_rows = int((inc > 0).sum().item())
    else:
        stage2_frob = 0.0
        stage2_changed_rows = 0

    final_stage = summarize_stage("final_v3", forget_records, retain_records, final_forget, final_retain, final_ppl)
    print(final_stage)

    gen_summary, gen_pairs = paired_gen_analysis(base_forget, stage1_forget, final_forget)
    ...
    head_summary = {
        ...
        "stage2_changed_rows_within_stage1_set": stage2_changed_rows,
        ...
        "stage1_deltaW_row_norm": percentile_summary(stage1_row_norms[stage1_changed].tolist()),
        "final_deltaW_row_norm": percentile_summary(final_row_norms[final_changed].tolist()),
    }
    ...
    report = {
        ...
        "seed": a.seed,
        "paths": {
            "base": str(Path(a.base_model_path).resolve()) if Path(a.base_model_path).exists() else a.base_model_path,
        ...
        "gen": {
            "base_token_position": token_position_summary(base_forget, "atomic_gen"),
            "stage1_token_position": token_position_summary(stage1_forget, "atomic_gen"),
            "final_token_position": token_position_summary(final_forget, "atomic_gen"),
    ...
    # Compact console digest for immediate diagnosis.
    print("\n===== V3 FORENSICS DIGEST =====")
    for row in report["stage_decomposition"]:
        print("{name:10s} Eff={Eff:.6f} Gen={AtomicGen:.6f} Spe={RetainEff:.6f} PPL={PPL}".format(**row))
    print("Head edits:", head_summary)
    print("Final AtomicGen by token position:", report["gen"]["final_token_position"])
