...
if [[ "$#" -ne 3 ]]; then
  echo "Usage: bash scripts/run_rwku_directional_sure_variant_b.sh MODEL CORPUS_DIR WIKIPEDIA_DIR" >&2
  exit 2
fi
...
CONFIG="config/rwku/directional_sure_variant_b_seed0.json"
OUTPUT_ROOT="${RWKU_DIRECTIONAL_SURE_VARIANT_B_OUTPUT_ROOT:-outputs/rwku_directional_sure_variant_b}"
...
if [[ -e "$RUN" ]]; then
  echo "Refusing to overwrite Directional SURE Variant B run: $RUN" >&2
  exit 2
fi
...
python scripts/rwku_directional_sure_variant_b.py \
  --model-path "$MODEL" \
...
echo "RWKU Directional SURE Variant B development run complete: $RUN"
