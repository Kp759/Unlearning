...
def apply_variant(
    ...
    if new_lm_overlap_scale is None:
        new_lm_overlap_scale = new_lm_scale
    ...
    for tid in target_new_ids:
        if 0 <= tid < lm.weight.shape[0]:
            lm_scale = new_lm_overlap_scale if tid in retain_ids else new_lm_scale
            lm_before = float(torch.norm(lm.weight[tid].detach().float()).cpu())
            lm.weight[tid].mul_(lm_scale)
            ...
            changes[str(tid)] = {
                ...
                "lm_head_scaled": True,
                "new_lm_scale": float(lm_scale),
                "new_lm_scale_default": float(new_lm_scale),
                "new_lm_overlap_scale": float(new_lm_overlap_scale),
                "lm_head_zeroed": float(lm_scale) == 0.0,
...
def main():
    ...
    ap.add_argument("--new-lm-scale", type=float, default=0.0)
    ap.add_argument("--new-lm-overlap-scale", type=float, default=None)
    ...
    changes = apply_variant(
        ...
        new_lm_scale=args.new_lm_scale,
        new_lm_overlap_scale=args.new_lm_overlap_scale,
    )
