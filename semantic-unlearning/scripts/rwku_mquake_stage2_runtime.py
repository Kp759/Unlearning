#!/usr/bin/env python3
"""Runtime setup for RWKU MQuAKE-style Stage 2."""
...
def prepare_runtime(args: argparse.Namespace, cfg: Mapping[str, Any], level1_anchor: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if not cases:
        raise RuntimeError("MQuAKE-style Stage2 runtime created no sensitive cases")
    ...
    with torch.no_grad():
        ...
        sparse.output_delta.copy_(
            level1_anchor["output_delta"].to(sparse.output_delta.device)
        )
    ...
    residual_rows = previous.residual_content_rows(
        tids_all, residual_case_indices, sensitive_rows
    )
    ...
    if float(initial_p["protected_non_sensitive_kl_mean"]) > 1e-7:
