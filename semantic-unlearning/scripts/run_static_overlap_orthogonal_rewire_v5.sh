#!/usr/bin/env bash
set -euo pipefail
model_path="${1:?Usage: bash scripts/run_static_overlap_orthogonal_rewire_v5.sh MODEL_PATH OVERLAP_MANIFEST}"
overlap_manifest="${2:?Provide the original overlap manifest.json}"
cd "$(dirname "${BASH_SOURCE[0]}")/.."
out="$PWD/outputs/static_overlap_orthogonal_rewire_v5_seed1"
...
python -u scripts/static_overlap_orthogonal_protocol_v5.py \
  --development-protocol "$dev" --overlap-manifest "$overlap_manifest" \
  --output-dir "$out" 2>&1 | tee "$PWD/outputs/static_overlap_orthogonal_rewire_v5_seed1.prepare.log"
python -u scripts/run_static_overlap_orthogonal_rewire_v5.py \
  --pilot-protocol "$out/pilot_protocol.json" --model-path "$model_path" \
  --device cuda --local-files-only 2>&1 | tee "$out.training.log"
python -u scripts/evaluate_static_overlap_orthogonal_rewire_v5.py \
