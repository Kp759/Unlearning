...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/tofu_sure_author_balanced_v4_baseemb_3b_test}"
...
# Same deterministic Stage1A as prior SURE runs.
...
# V4 deliberately removes the extra NLL buffer.  The declared forget-quality
# requirement remains answer_probability <= 3e-4, enforced on all 50 direct QAs.
...
# Delete only SURE checkpoint weight directories.  All JSON/JSONL/log/report
# files from prior experiments remain in their parent directories.
...
for SEED in ${SEEDS}; do
  ...
  STAGE1B="${ROOT}/stage1b_progressive_v4_baseemb"
  mkdir -p "${ROOT}"
  ...
  echo
  echo "===== SURE-TOFU-v4 SEED ${SEED}: STAGE1A SAME-PROMPT GA/GD ====="
  ...
  echo "===== V4: ALL ANSWER EMBEDDINGS->BASE + SPARSE LM RANK0 + ZERO BUFFER ====="
  if [[ "${SKIP_EXISTING}" != "1" || ! -f "${STAGE1B}/checkpoint/model.safetensors" ]]; then
    rm -rf "${STAGE1B}/checkpoint"
    python scripts/tofu_sure_rank0_forget_progressive_v4.py \
  ...
  echo "===== V4 CHECKPOINT FROZEN; LOCKED HELD-OUT EVALUATION STARTS ====="
  EVAL_ARGS=(
    --model-dir "${STAGE1B}/checkpoint"
    --eval-dir "${EVAL_DIR}"
    --reference-model-dir "${FULL_TOFU_MODEL}"
    --output "${ROOT}/locked_eval_v4_r0.json"
    --seed "${SEED}"
    --dtype "${DTYPE}"
...
if [[ ! -f "${FULL_TOFU_MODEL}/model.safetensors" || ! -f "${FULL_TOFU_MODEL}/config.json" ]]; then
  echo "FATAL: protected Full-TOFU checkpoint missing after v4 cleanup" >&2
  exit 93
fi

echo
echo "SURE-TOFU-v4 complete."
echo "All visible answer embeddings were restored to Base; only sparse sensitive LM-head rows were editable."
echo "Direct-forget NLL buffer was zero; boundary bisection targeted the declared probability threshold."
