...
MODEL="${1:?Usage: bash scripts/run_mcf_sure_target_aware.sh MODEL [MCF_JSON]}"
MCF="${2:-data/multi_counterfact.json}"
WIKIDATA_DIR="${WIKIDATA_DIR:-data/wikidata}"
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mcf_sure_target_aware_true_ga_new_gd_v7}"
...
# Reuse the same external Wikipedia corpus, guard budgets, and numerical
# settings as neutral SURE. The benchmark-aware supervision is isolated below.
...
for SEED in "${SEEDS[@]}"; do
  ...
  LEARNER="${ROOT}/target_aware_learner"
  ...
  # The manifest locks the exact official sample. The target-aware learner then
  # reads target_new and paraphrases from the original MCF source by those IDs.
  ...
  python scripts/sure_mcf_target_aware_two_stage.py \
    --model-path "${MODEL}" \
    --mcf-path "${MCF}" \
  ...
  # Everything below is a post-training audit. The learner has already applied
  # its own checkpoint-dtype 100/100 acceptance gate before saving.
...
echo "MCF target-aware true-GA/new-GD SURE complete: ${OUTPUT_ROOT}"
