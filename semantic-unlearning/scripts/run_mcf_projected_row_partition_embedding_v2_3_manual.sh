...
# Training-only MCF V2.3. The split builder alone sees MCF_PATH.
# Required environment: MODEL_PATH, WIKIDATA_DIR, MCF_PATH
# Usage: bash scripts/run_mcf_projected_row_partition_embedding_v2_3_manual.sh OUTPUT_DIR
#
# Frozen LM head. Only selected input-embedding rows may change. Set
# SURGICAL_WEIGHT to a positive value to run the registered answer-contrast
# ablation instead of the primary arm.

OUTPUT_DIR=${1:?fresh V2.3 output directory required}
...
if [ -e "$OUTPUT_DIR" ]; then
  echo "ERROR: V2.3 output already exists: $OUTPUT_DIR" >&2
  exit 1
fi

REGISTRY=protocols/mcf_projected_row_partition_embedding_v2_3_registry.json

if [ "$SURGICAL_WEIGHT" != "0.0" ]; then
  echo "NOTE: surgical weight $SURGICAL_WEIGHT selects the registered" >&2
  echo "      answer-contrast ablation, not the primary V2.3 arm." >&2
fi
...
printf '\nMCF V2.3 training-only run finished.\n'
printf 'Row partition: %s\n' "$OUTPUT_DIR/method/row_partition.json"
