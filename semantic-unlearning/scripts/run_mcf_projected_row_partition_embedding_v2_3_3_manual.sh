...
# Training-only MCF V2.3.3. The split builder alone sees MCF_PATH.
# Required environment: MODEL_PATH, WIKIDATA_DIR, MCF_PATH
# Usage: bash scripts/run_mcf_projected_row_partition_embedding_v2_3_3_manual.sh OUTPUT_DIR

OUTPUT_DIR=${1:?fresh V2.3.3 output directory required}
...
if [ -e "$OUTPUT_DIR" ]; then
  echo "ERROR: V2.3.3 output already exists: $OUTPUT_DIR" >&2
  exit 1
fi

REGISTRY=protocols/mcf_projected_row_partition_embedding_v2_3_3_registry.json
...
python -u scripts/run_mcf_projected_row_partition_embedding_v2_3.py \
  ...
  --sensitivity-forget-records 50 \
  --sensitivity-retain-records 2000 \
  --sensitivity-coverage-relative-epsilon 0.0001 \
  --forget-importance-floor-relative 0.01 \
  --importance-ratio-min 1.0 \
  --forget-specific-ratio-min 4.0 \
  --forget-specific-retain-coverage-max 0.01 \
  --retain-tail-ratio-min 0.25 \
  --require-hybrid-role-policy \
  --require-per-prompt-reachability \
  --require-iterative-reachability \
  --iterative-reachability-steps 64 \
  --iterative-reachability-step-fraction 0.02 \
  --iterative-reachability-minimum-improvement 0.000001 \
...
printf '\nMCF V2.3.3 training-only run finished.\n'
printf 'Sensitivity: %s\n' "$OUTPUT_DIR/method/per_example_row_sensitivity.json"
printf 'Guard bank: %s\n' "$OUTPUT_DIR/method/guarded_retain_bank.json"
