...
if [[ "$#" -ne 3 ]]; then
  echo "Usage: bash scripts/run_rwku_directional_sure_v2_emb_ga_only.sh MODEL CORPUS_DIR WIKIPEDIA_DIR" >&2
  exit 2
fi
...
CONFIG="config/rwku/directional_sure_v2_emb_ga_only_seed0.json"
OUTPUT_ROOT="${RWKU_DIRECTIONAL_SURE_V2_EMB_GA_ONLY_OUTPUT_ROOT:-outputs/rwku_directional_sure_v2_emb_ga_only}"
...
if [[ -e "$RUN" ]]; then
  echo "Refusing to overwrite embedding-GA-only run: $RUN" >&2
  exit 2
fi
...
python scripts/rwku_directional_sure_v2_emb_ga_only.py \
  --model-path "$MODEL" \
...
echo "RWKU Directional SURE v2 embedding-GA-only development run complete: $RUN"
