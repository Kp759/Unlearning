...
if [[ "$#" -ne 4 ]]; then
  echo "Usage: bash scripts/run_rwku_sure_hidden_direction_v32_kl_w1k.sh MODEL STEPHEN_KING_ATOMIC_CORPUS_DIR REAL_WIKIPEDIA_DIR SOURCE_HEAD_ONLY_RUN_DIR" >&2
  exit 2
fi
...
output_root="${RWKU_HD_V32_KL_W1K_OUTPUT_ROOT:-outputs/rwku_hd_v32_kl_w1k}"
...
if [[ -e "${run_dir}" ]]; then
  echo "Refusing to overwrite v3.2 KL run: ${run_dir}" >&2
  echo "Choose a new RWKU_HD_V32_KL_W1K_OUTPUT_ROOT; never reuse a checkpoint receipt." >&2
  exit 2
fi
...
"${python_bin}" scripts/rwku_sure_hidden_direction_v32_kl_w1k.py \
  --model-path "${model}" \
...
echo "RWKU-H+HD-v3.2-KL-W1K checkpoint frozen: ${run_dir}"
echo "Development-only Stephen King v3.2. Uses 1000 optimization Wiki contexts with exact KL and a disjoint 1000-context exact-KL final gate."
