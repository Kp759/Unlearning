...
if [[ $# -ne 8 ]]; then
  echo "usage: $0 CLEAN_WRITER_OUTPUT_DIR V3_2_OUTPUT_DIR V3_4_OUTPUT_DIR REJECTED_V3_5_OUTPUT_DIR V3_5_1_FORENSICS_OUTPUT_DIR REJECTED_V3_5_2_OUTPUT_DIR REJECTED_V3_5_3_OUTPUT_DIR OUTPUT_DIR" >&2
  exit 2
fi
...
if [[ -e "${RUN_OUTPUT_DIR}" ]]; then
  echo "output path already exists; choose a fresh V3.5.4 directory: ${RUN_OUTPUT_DIR}" >&2
  exit 2
fi
...
exec bash "${PROJECT_DIR}/slurm/run_mcf_embedding_keyed_neuron_v3_5_4_balanced_multilabel_repair_seed1_3b.slurm"
