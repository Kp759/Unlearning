...
TARGET_REPRESENTATION_OUT_DIR="${TARGET_REPRESENTATION_OUT_DIR:-$PWD/results/retain_anchored_context_head/mcf/seed1_target_representation_compare_fix5f}"

python scripts/mcf_target_representation_compare_fix5f_seed1.py \
  --model-path "$MODEL_PATH" \
  --mcf-path "$MCF_PATH" \
  --view-corpus-fix5 "$RELATION_V2_CORPUS" \
  --output-dir "$TARGET_REPRESENTATION_OUT_DIR" \
  --dtype "${DTYPE:-bf16}" \
  --device "${DEVICE:-cuda}" \
