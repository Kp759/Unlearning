...
The method reached `Eff=0` and `Gen=0` on **10/10 seeds** and passed the full strict gate on **9/10 seeds**. Seed 7 remained a failure because its minimum margin was `0.09375`, which is `0.00625` below the frozen threshold.
...
Canonical machine-readable record: `config/best_runs/mcf/official_protected_v2_seeds0_9.json`.
Canonical Markdown record: `config/best_runs/mcf/official_protected_v2_seeds0_9.md`.

## Checkpoint storage

Model weights remain on Wulver. Exact training configs and SHA-256 hashes still need to be captured from persistent storage before publication-grade reproducibility is claimed.
