...
## NPO + retain CE + retain KL

`--forget-loss-type npo` adds a training-based unlearning objective that compares the current policy against a frozen reference/base model. For each forget example, it treats the negative response as `target_new` on MCF and as the forget answer on TOFU. It computes answer-only sequence log-probabilities for the current model and reference model, then minimizes:

```text
log_ratio = policy_logp - ref_logp
npo_loss = (2 / beta) * softplus(beta * log_ratio)
```

Minimizing NPO pushes the current model to assign lower probability to the negative response than the reference model. This differs from plain `answer_nll` GA/GD, which directly maximizes forget answer NLL, and from `mcf_margin`, which directly pushes MCF `target_new_nll >= target_true_nll`. Retain CE still trains retain answers, while `--kl-retain-weight` adds token-level retain KL against the frozen reference model. Use `--kl-retain-weight > 0` for NPO+retain-KL; the script allows KL-free NPO but prints a warning.
...
The four-scope MCF runner is:

```bash
cd semantic-unlearning
bash scripts/run_npo_retainkl_4scope_mcf.sh \
  /scratch/yl258/kp759/hf/models--meta-llama--Llama-3.2-3B-Instruct/snapshots/0cb88a4f764b7a12671c53f0838cd831a0843b95
```

It writes the final official-compatible table to:

```text
outputs/npo_retainkl_4scope_mcf/same_eval_with_base/official_eval_comparison.md
```

Metric directions: lower forget negative-response probability/log-probability is stronger unlearning; for official MCF evaluation, Eff ↓ and Gen ↓ are better unlearning, Spe ↑ is better specificity, and PPL ↓ or stable is better fluency. If two model copies do not fit on GPU, rerun with `--reference-device cpu` or set `REFERENCE_DEVICE=cpu` for the runner.
